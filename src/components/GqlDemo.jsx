...
const GQL =
  "https://jsramverk-mapg23-e4hebqhcfxbpcbey.northeurope-01.azurewebsites.net/graphql/auth"; // <- change port if needed
...
export default function GqlDemoPro() {
  ...
  async function onCreate() {
    ...
    try {
      ...
      if (addDocument.status !== 200)
        throw new Error(addDocument.msg || "Add failed");
      await loadDocs();
  ...
  async function onSelect(d) {
    ...
    setMsg("");
    try {
      await loadDoc(d.ownerId, d.index);
    } catch (e) {
      setErr(String(e.message || e));
    }
  }
  ...
  async function onSave() {
    ...
    try {
      const { updateDocument } = await gql(
        ...
        {
          id: selected.ownerId,
          i: Number(selected.index),
          t: form.title,
          c: form.content,
        }
      );
      if (updateDocument.status !== 200)
        throw new Error(updateDocument.msg || "Update failed");
  ...
  async function onShare() {
    ...
    try {
      ...
      if (shareDocument.status !== 200)
        throw new Error(shareDocument.msg || "Share failed");
      ...
      {!me && (
        ...
        >
          ...
          <input
            ...
            onChange={(e) =>
              setLoginForm((v) => ({ ...v, email: e.target.value }))
            }
          />
          <input
              ...
              <ul>
                {shared.map((d) => (
                  <li
                    ...
                    onClick={() => onSelect(d)}
                  >
                    <b>{d.title || "(untitled)"}</b> — idx {d.index} (owner{" "}
                    {d.ownerId.slice(0, 6)}…)
                  </li>
                ))}
              </ul>
              ...
              {selected && (
                <>
                  <div style={{ display: "grid", gap: 8, maxWidth: 520 }}>
                    ...
                    <small>
                      ownerId: <code>{selected.ownerId}</code> | index:{" "}
                      <code>{String(selected.index)}</code>
                    </small>
                  </div>
                </>
              )}
